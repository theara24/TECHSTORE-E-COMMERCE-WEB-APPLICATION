    ...
    <nav className="glass sticky top-0 z-50 px-6 py-4">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-10 font-semibold text-sm tracking-wide">
          <Link to="/" className={`${isActive('/') ? 'text-blue-600' : 'text-slate-600'} hover:text-blue-600 transition-colors uppercase`}>Home</Link>
          <Link to="/products" className={`${isActive('/products') ? 'text-blue-600' : 'text-slate-600'} hover:text-blue-600 transition-colors uppercase`}>Shop</Link>
          <Link to="/about" className={`${isActive('/about') ? 'text-blue-600' : 'text-slate-600'} hover:text-blue-600 transition-colors uppercase`}>Our Story</Link>
          <Link to="/contact" className={`${isActive('/contact') ? 'text-blue-600' : 'text-slate-600'} hover:text-blue-600 transition-colors uppercase`}>Contact</Link>
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white border-t border-slate-100 p-6 flex flex-col gap-6 font-bold shadow-xl animate-in slide-in-from-top duration-300">
          <Link to="/" className={`${isActive('/') ? 'text-blue-600' : ''}`} onClick={() => setIsMenuOpen(false)}>Home</Link>
          <Link to="/products" className={`${isActive('/products') ? 'text-blue-600' : ''}`} onClick={() => setIsMenuOpen(false)}>Shop</Link>
          <Link to="/about" className={`${isActive('/about') ? 'text-blue-600' : ''}`} onClick={() => setIsMenuOpen(false)}>Our Story</Link>
          <Link to="/contact" className={`${isActive('/contact') ? 'text-blue-600' : ''}`} onClick={() => setIsMenuOpen(false)}>Contact</Link>
